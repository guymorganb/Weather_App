...
// 2nd half of first api call using lattitue and longitude to target the dity
function findCityData(city){getData(city).then(function(weatherData){
    ...
    // get the future cast / (forecast)
    getFutureCast().then(function(futureCast){
        console.log(futureCast);
         // sorts the list inside the json object in asscending order by the dt_txt property (not entirely needed but in case the data order changes)
         futureCast.list.sort(function(a, b) {     
            ...
            // loops through the futureCast of 6 days and append items from api
...
// adds inputs into local storage and generates a key automatically for sorting later
...
// cleans dynamic elements so new can be populated, sets a time to delay the elements to be update
...
// pulls data from local storage checks inputs
...
// gets the text inside the gray button and cleans it so it can be reused to call up the API
...
// clears out local storage and the list of buttons
function clearAll(){
    localStorage.clear();
...
// function to remove a sinlge button when red box is clicked, also removes from local storage
...
// entry point of the script sequence
function init(){
    let srcBtn = $('.search');
...
// wait for page load, and setup execution 
